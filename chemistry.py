"""Added the atomic number for each element, added the sum_proton function, and modified the main function to print the total number of protons"""
...
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    for symbol, quantity in symbol_quantity_list:
        if symbol in periodic_table_dict:
            atomic_number = periodic_table_dict[symbol][2]  # Index 2 for atomic number
...
def main():
    chemical_formula = input("Enter the chemical formula of the molecule (e.g., H2O, C6H12O6): ")
    ...
    number_of_moles = sample_mass / molar_mass if molar_mass else 0
    total_protons = sum_protons(symbol_quantity_list, periodic_table)
    print(f"The molar mass of {chemical_formula}: {molar_mass:.2f} grams/mole")
    print(f"The number of moles in the sample: {number_of_moles:.5f}")
    print(f"Total number of protons in {chemical_formula}: {total_protons}")
